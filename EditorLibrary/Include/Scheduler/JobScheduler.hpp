...
struct JobHandle {
	...
	// Comparison operator
	inline bool operator==(const JobHandle& rhs) const {
		return _jobID == rhs._jobID;
	}
	...
	inline JobHandle(const JobHandle& rhs) {
		*this = rhs;
	}
...
inline JobDescriptor MakeJob(JobFunction Fn, void* ArgA, void* ArgB, JobPriority P = JOB_PRIORITY_NORMAL) {
	return JobDescriptor{ Fn, P, ArgA, ArgB };
}
...
public:
	...
	/// You cannot wait on any of the same jobs from multiple threads! I.e only call once for each set of job handles, otherwise an assert will be fired.
	/// Note that there should NOT be any duplicate handles! Otherwise an assert will be thrown.
	/// </summary>
...
private:
	...
	/// <summary>
	/// Internal: Decrement the number of references to the Job. If its now zero, the internal counter is freed.
	/// </summary>
	void _DecrementRefs(U32 jobID);
	...
	std::atomic<Bool> _running; // true if we are currently running, false otherwise. cancelJobs is protected by this atomic by acq/rel, order matters (this one first)
	Bool _cancelJobs; // if running is false and this is true, then cancel any jobs and exit quick asap. if running false and this false, finish job queues.
...
JobHandle::~JobHandle()
{
	if (IsValid() && JobScheduler::Instance) // Ensure scheduler is alive. If not, no need to decrement
		JobScheduler::Instance->_DecrementRefs(_jobID); // Handle is finished with, decrement number of references
}
