...
function RegisterTXD3DTexture(vendor)
    local tex = NewClass("class D3DTexture", 0)
    tex.Extension = "d3dtx"
    tex.Serialiser = "SerialiseD3DTexture_TX"
    tex.Normaliser = "NormaliseD3DTexture_TX"
    ...
    tex.Members[4] = NewMember("mNumMipLevels", kMetaUnsignedInt)
    tex.Members[5] = NewMember("mD3DFormat", kMetaUnsignedInt)
    tex.Members[6] = NewMember("mWidth", kMetaUnsignedInt)
    tex.Members[7] = NewMember("mHeight", kMetaUnsignedInt)

    tex.Members[8] = NewMember("_TextureData", kMetaClassInternalBinaryBuffer)
    tex.Members[8].Flags = kMetaMemberVersionDisable
    MetaRegisterClass(tex)
    return tex
end

function NormaliseD3DTexture_TX(instance, state) 
...
function SerialiseD3DTexture_TX(stream, instance, write)
