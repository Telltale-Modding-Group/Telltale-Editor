...
enum MetaMemberFlag
{
    MEMBER_BLOCKED = 0, // The member has a block size around it.
    MEMBER_ENUM = 1,    // The member is an enum.
    MEMBER_FLAG = 2,    // The member is a flags bitfield.
};

enum MetaClassFlag
{
    CLASS_INTRINSIC = 0, // The is an intrinsic type so is not added to meta class header.
};
