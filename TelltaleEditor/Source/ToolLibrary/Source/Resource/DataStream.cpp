...
static void DataStreamDeleter(DataStream* Stream){
    TTE_DEL(Stream);
}
...
std::shared_ptr<DataStream> DataStreamManager::CreateTempStream() { 
    return CreateFileStream(ResourceURL(ResourceScheme::FILE, FileNewTemp()));
}
...
U64 DataStreamFile::GetSize() { 
    return _Handle == FileNull() ? 0 : FileSize(_Handle);
}
...
std::shared_ptr<DataStream> ResourceURL::Open() {
    if(_Scheme == ResourceScheme::INVALID)
        ...
        try {
            ...
            }else return nullptr; // No errors, it just could not be resolved.
...
String ResourceURL::GetPath() const { 
    return SchemeToString(_Scheme) + ":" + _Path;
}
...
ResourceURL::ResourceURL(const String &path) { 
    ...
    if(pos == std::string::npos) {// no ':', default file.
        _Path = path;
...
void ResourceURL::_Normalise() {
    ...
    _Path.erase(std::remove_if(_Path.begin(), _Path.end(), [&validChars](char c) {
...
void DataStreamMemory::_EnsureCap(U64 bytes){//bytes is target size
