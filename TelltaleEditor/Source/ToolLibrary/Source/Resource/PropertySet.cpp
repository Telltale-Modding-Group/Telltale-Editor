...
{
    Meta::ClassInstance prop = luaProp(man, 1);
...
static U32 luaPropertyCreate(LuaManager& man)
{
    ...
    Meta::ClassInstance value = Meta::CreateInstance(clz, prop, kn);
    if(man.GetTop() == 4)
    {
...
static U32 luaPropertySet(LuaManager& man)
{
    ...
    if(value && !Meta::CoerceLuaToMeta(man, 3, value))
        TTE_LOG("Failed to assign property %s from script: argument type was invalid", GetRuntimeSymbols().FindOrHashString(kn).c_str());
    return 0;
}
...
static U32 luaPropertyGet(LuaManager& man)
{
    ...
    {
        man.PushNil();
        TTE_LOG("Error coercing meta instance to lua for class %s", Meta::GetClass(prop.GetClassID()).Name.c_str());
    }
    return 1;
}
...
{
    Meta::ClassInstance dest = luaProp(man, 1), source = luaProp(man, 2);
    PropertySet::ImportKeysValuesAndParents(dest, source, true, true, man.GetTop() == 3 ? luaProp(man, 3) :
                                            Meta::ClassInstance{}, true, false, ResourceRegistry::GetBoundRegistry(man));
    return 0;
}
...
{
    Meta::ClassInstance prop = luaProp(man, 1);
    Meta::ClassInstance keysFrom = luaProp(man, 2);
...
void PropertySet::InjectScriptAPI(LuaFunctionCollection& Col)
{
    ...
    PUSH_FUNC(Col, "PropertyGetGlobals", &luaPropertyClearKeys, "table PropertyGetGlobals(propertySet)", "Gets all global parent property file names.");
    ...
    PUSH_FUNC(Col, "PropertyCreate", &luaPropertyCreate, "nil PropertyCreate(propertySet,keyName,typeName)",
              "Add a value to the property set. Valid type names are: \"bool\", \"class Color\", etc. Careful. 'class ' etc are not used in new games."
              " Optional 4th argument is the property value to assign.");
...
void PropertySet::Set(Meta::ClassInstance prop, Symbol key, Meta::ClassInstance value, Ptr<ResourceRegistry> pRegistry, SetPropertyMode mode)
{
    ...
    else if(mode == SetPropertyMode::COPY)
...
void PropertySet::MarkModified(Meta::ClassInstance prop, Symbol Key, Ptr<ResourceRegistry> pRegistry)
{
    ...
    if(it != callbacks.end())
    {
        ...
        while(callback)
        {
            callback->CallMeta(std::move(value), {}, {}, {});
