...
static void ImGui_ImplSDLGPU3_SetupRenderState(ImDrawData* draw_data, SDL_GPUGraphicsPipeline* pipeline, SDL_GPUCommandBuffer* command_buffer, SDL_GPURenderPass* render_pass, ImGui_ImplSDLGPU3_FrameData* fd, uint32_t fb_width, uint32_t fb_height)
{
	...
	struct UBO
	{
		float scale[2]; float translation[2];
	} ubo;
...
fragment float4 fragment_main(VertexOutput in [[stage_in]],
                              texture2d<float> sTexture [[texture(0)]],
                              sampler sSampler [[sampler(0)]])
{
    float4 texColor = sTexture.sample(sSampler, in.UV);
                                ...
                                constant Uniforms& ubo [[buffer(0)]])
{
    ...
    out.Color = in.aColor;
    out.UV = in.aUV;
    out.Position = float4(in.aPos * ubo.uScale + ubo.uTranslate, 0.0, 1.0);
    out.Position.y *= -1.0;
    return out;
}
)";
	...
	else
	{
		...
		vertex_shader_info.code = (const Uint8*)VERT_MSL.c_str();
		vertex_shader_info.code_size = VERT_MSL.length();
		fragment_shader_info.entrypoint = "fragment_main";
		fragment_shader_info.format = SDL_GPU_SHADERFORMAT_MSL;
		fragment_shader_info.code = (const Uint8*)FRAG_MSL.c_str();
		fragment_shader_info.code_size = FRAG_MSL.length();
	}
#endif
...
static void ImGui_ImplSDLGPU3_CreateGraphicsPipeline()
{
    ...
    //pipeline_info.props = props;
    //SDL_SetBooleanProperty(props, "NonTTE", true);
...
void ImGui_ImplSDLGPU3_DestroyDeviceObjects()
{
	...
	if (bd->VertexShader)
	{
		SDL_ReleaseGPUShader(v->Device, bd->VertexShader); bd->VertexShader = nullptr;
	}
	if (bd->FragmentShader)
	{
		SDL_ReleaseGPUShader(v->Device, bd->FragmentShader); bd->FragmentShader = nullptr;
	}
	if (bd->FontSampler)
	{
		SDL_ReleaseGPUSampler(v->Device, bd->FontSampler); bd->FontSampler = nullptr;
	}
	if (bd->Pipeline)
	{
		SDL_ReleaseGPUGraphicsPipeline(v->Device, bd->Pipeline); bd->Pipeline = nullptr;
	}
}
